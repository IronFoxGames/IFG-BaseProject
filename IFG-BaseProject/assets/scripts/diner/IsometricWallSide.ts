...
//
@ccclass('IsometricWallSide')
export class IsometricWallSide extends ModelComponent {
    @property(Texture2D)
    ...
    private _initializeSharedResources() {
        ...
        const meshInfo = {
            positions: [
                ...
                //
                0 * mirrorFactor, 0, 0,
                this.wallWidth * mirrorFactor * -halfCellWidth, this.wallWidth * halfCellHeight, 0,
                this.wallWidth * mirrorFactor * -halfCellWidth, this.wallWidth * halfCellHeight + wallHeight, 0,
    ...
    public updateMaterial() {
        ...
        const binding = pass.getBinding(IsometricWallSide._texturePropertyName);
        if (typeof binding === 'number') {
            pass.bindTexture(binding, target.getGFXTexture());
        }

        this.material.setProperty(IsometricWallSide._tilingOffsetPropertyName, this.tilingOffset);

        this.material.setProperty(IsometricWallSide._blendColorPropertyName, this.blendColor);

        this.material.setProperty(IsometricWallSide._maskingPropertyName, this.masking);
    }
